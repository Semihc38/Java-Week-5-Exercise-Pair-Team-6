...
SELECT name, daily_fee FROM campground GROUP BY name, daily_fee ORDER BY name;
-- select name, open from month, open to month, and daily fee of the campgrounds where daily fee is less than $100
SELECT name, open_from_mm, open_to_mm, daily_fee FROM campground WHERE daily_fee::numeric < 100;
...
SELECT name, description FROM park GROUP BY name, description, establish_date ORDER BY establish_date DESC;
...
SELECT reservation_id, site_id, name, from_date, to_date FROM reservation WHERE from_date BETWEEN '10/01/2020' AND '10/31/2020';
